...
public abstract class BaseClient :IDisposable
{
   ...
   // ~BaseClient() => Dispose(false); //финализатор
    public void Dispose()
    {
        Dispose(true);
       GC.SuppressFinalize(this); // если есть финализатор (~BaseClient() { ... }) - удалить данный объект из очереди на финализацию если был вызван данный метод
    }
    ...
    protected virtual void Dispose(bool Disposing)
    {
        ...
        if (Disposing)
        {
            // здесь должны очистить управляемые ресурсы
            //Http.Dispose(); // у объекта вызвать метод Dispose() нельзя -  его не создавали тут
        }
